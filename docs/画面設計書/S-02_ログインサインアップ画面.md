...
- ログアウトはNextAuthのセッション破棄（`signOut()`）のみを実装した。Cognitoのグローバルログアウトエンドポイント呼び出しは未実装（今後の改善余地）
- Phase 1（ローカル開発、`app.auth.mode: no-auth`）では認証を行わず固定devユーザーで動作する。Phase 2（本実装）はCognito Hosted UIリダイレクトで動作する（[ielts-createrリポジトリ docs/ロードマップ.md](https://github.com/h-fujiwara-dev/ielts-creater/blob/main/docs/ロードマップ.md)参照）
